        ...
        public void TestCLickCart()
        {
            ...
            if (productDivs.Count > 0)
            {
                ...
                if (buttons.Count > 0)
                {
                    ...
                    // Nhấp vào thẻ <li>
                    cartItem.Click();
                    IWebElement plusButton = wait.Until(d => d.FindElement(By.XPath("//button[contains(text(), '+')]")));
                    plusButton.Click();
                    Thread.Sleep(2000);
                    IWebElement saveButton = driver.FindElement(By.XPath("//button[@type='button' and span[text()='Lưu']]"));
                    saveButton.Click();
                    Thread.Sleep(4000);
                    IWebElement okButton3 = wait.Until(d => d.FindElement(By.CssSelector(".ant-btn-primary")));
                    okButton3.Click();
                    Thread.Sleep(5000);
                    IList<IWebElement> productItems = driver.FindElements(By.CssSelector("ul.ant-list-items li.ant-list-item"));
