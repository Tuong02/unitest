        ...
        public void TestUpdateProduct()
        {
            ...
            using (FileStream fileStream = new FileStream(filePath, FileMode.Open, FileAccess.Read))
            {
                ...
                for (int i = 1; i <= sheet.LastRowNum; i++) // Bắt đầu từ 1 nếu hàng đầu tiên là tiêu đề
                {
                    ...
                    if (row != null)
                    {
                        IWebElement twoRow = wait.Until(d => d.FindElement(By.XPath("//tbody/tr[1]")));
                        //Thực hiện update sản phẩm
                        IWebElement detailButtonTwo = driver.FindElement(By.XPath(".//button[contains(., 'Chi tiết')]"));
                        ...
                        //IWebElement changeButtonUpdate = driver.FindElement(By.CssSelector(".label__change__image input[type='file']"));
                        //changeButtonUpdate.SendKeys(@"C:\Users\pcgosei\Downloads\img.png");

                        IWebElement dateInputUpdate = driver.FindElement(By.CssSelector("input[placeholder='Select date']"));

                        string currentDateTimeUpdate = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

                        js.ExecuteScript("arguments[0].value='" + currentDateTimeUpdate + "'; arguments[0].dispatchEvent(new Event('input'));", dateInputUpdate);
                        js.ExecuteScript("arguments[0].dispatchEvent(new Event('change'));", dateInputUpdate);

                        IWebElement addButtonUpdate = wait.Until(d => d.FindElement(By.XPath("//button[span[text()='Update']]")));
                        addButtonUpdate.Click();
                        Thread.Sleep(1000);

                        // Kiểm tra kết quả 
                        Thread.Sleep(1000);

                        if (nameField.GetAttribute("value")!=null && weightField.GetAttribute("value").Length > 0 && brandField.GetAttribute("value").Length > 0 && priceField.GetAttribute("value").Length > 0 && detailField.GetAttribute("value").Length > 0)
                        {
                            ...
                            else
                            {
                                ...
                                row.CreateCell(6).SetCellValue("Fail_Hệ thống vẫn cho phép nhập quá 50 kí tự vào các trường thông tin"); 
                            }
                        }
                        else
                        {
                            Console.WriteLine("Hệ thống vẫn cho để trống các trường thông tin");
                            row.CreateCell(6).SetCellValue("Fail_Hệ thống vẫn cho phép để trống các trường thông tin"); 
                        }
